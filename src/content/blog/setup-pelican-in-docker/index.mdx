...
Pelican is the successor to [Pterodactyl](https://pterodactyl.io/) and is intended to improve on aspects of game server management where Pterodactyl fell short. Pelican is currently in Beta and this guide is currently up-to-date with Panel [v1.0.0-beta31](https://github.com/pelican-dev/panel/releases/tag/v1.0.0-beta31) and Wings [v1.0.0-beta22](https://github.com/pelican-dev/wings/releases/tag/v1.0.0-beta22).
...
```bash
cd ~/panel
git switch release/v1.0.0-beta31
...
Next, build the Docker images for both repositories and tag them with a useful name. These image names must be reflected in the `docker-compose.yml` that we will create. Each build takes a few minutes to complete. Note that `panel` has a special `Dockerfile.dev` that is used for building the image locally.
